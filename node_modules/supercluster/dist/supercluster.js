...
Supercluster.prototype.getClusters = function getClusters (bbox, zoom) {
    ...
    for (var i = 0, list = ids; i < list.length; i += 1) {
        ...
        // console.warn(c.id)
        var dd = c && c.id ? this.getLeaves(c.id) : null  
        // console.warn(c)
        clusters.push(c.numPoints ? getClusterJSON(c,dd) : this.points[c.index]);
    }
...
Supercluster.prototype._cluster = function _cluster (points, zoom) {
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        if (numPoints >= minPoints) { // enough points to form a cluster
            ...
            const ff =this.getLeaves(id)
        
            clusters.push(createCluster(wx / numPoints, wy / numPoints, id, numPoints, clusterProperties, ff));
...
function createCluster(x, y, id, numPoints, properties, ff) {
    return {
...
function getClusterJSON(cluster, dd) {
    let prp = getClusterProperties(cluster)
    prp.fak = dd
    return {
        type: 'Feature',
        id: cluster.id,
